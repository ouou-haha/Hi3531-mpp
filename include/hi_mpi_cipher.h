 ...
 * you need to config IV(config pstCtrl->stChgFlags.bits_IV with 1) by transferring HI_UNF_CIPHER_ConfigHandle.
 * param[in] handle           Cipher handle
...
hi_s32 hi_mpi_cipher_klad_encrypt_key(hi_cipher_ca_type root_key,
    hi_cipher_klad_target target, const hi_u8 *clean_key, hi_u8 *ecnrypt_key, hi_u32 key_len);
 ...
 * N/A
 */
hi_s32 hi_mpi_cipher_rsa_verify(const hi_cipher_rsa_verify *rsa_verify, const hi_cipher_verify_data *verify_data);
