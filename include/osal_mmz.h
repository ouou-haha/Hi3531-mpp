...
#define hil_mmb_fmt_arg(p) (p)->phys_addr, mmz_grain_align((p)->phys_addr + (p)->length) - 1, \
...
#define begin_list_for_each_mmz(p, gfp, mmz_name)        \
    list_for_each_entry(p, hil_mmz_get_mmz_list(), list)   \
    {                                                    \
        if ((gfp) == 0 ? 0 : (p)->gfp != (gfp)) {        \
            continue;                                       \
        }                                                   \
        if (((mmz_name) == NULL) || (*(mmz_name) == '\0')) { \
            if (hil_mmz_get_anony() == 1) {                \
...
hil_mmz_t *hil_mmz_create_v2(const char *name, const hil_mmz_create_attr *mmz_create_attr);
...
int hil_mmz_get_anony(void);
struct osal_list_head *hil_mmz_get_mmz_list(void);
