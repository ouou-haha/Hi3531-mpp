...
#define HI_MAXI_NUM_LIMIT 30000
    ...
    hi_s32 bit_width, hi_vb_calc_cfg *calc_cfg)
{
    ...
    if ((buf_attr->width > HI_MAXI_NUM_LIMIT) || (buf_attr->height > HI_MAXI_NUM_LIMIT)) {
        (hi_void)memset_s(calc_cfg, sizeof(*calc_cfg), 0, sizeof(*calc_cfg));
        return;
    }
    ...
    hi_u32 bit_width, hi_vb_calc_cfg *cfg)
{
    ...
    hi_u32 align_height;
    const hi_u32 cmp_ratio_luma   = HI_SEG_RATIO_8BIT_LUMA;
    const hi_u32 cmp_ratio_chroma = HI_SEG_RATIO_8BIT_CHROMA;
...
{
    if ((type == HI_PT_H265) && (width <= HI_VDH_H265D_MAX_WIDTH) && (height <= HI_VDH_H265D_MAX_HEIGHT)) {
        return HI_TRUE;
    }
    if ((type == HI_PT_H264) && (width <= HI_VDH_H264D_MAX_WIDTH) && (height <= HI_VDH_H264D_MAX_HEIGHT)) {
        return HI_TRUE;
    }
    if ((type == HI_PT_JPEG || type == HI_PT_MJPEG) &&
        (width <= HI_JPEGD_MAX_WIDTH) && (height <= HI_JPEGD_MAX_HEIGHT)) {
        return HI_TRUE;
    }
    return HI_FALSE;
}
    ...
    hi_pic_buf_attr *buf_attr)
{
    ...
    if (hi_vdec_check_pic_size(type, buf_attr->width, buf_attr->height) == HI_FALSE) {
        return 0;
    }
...
__inline static hi_u32 hi_vdec_get_tmv_buf_size(hi_payload_type type, hi_u32 width, hi_u32 height)
{
    ...
    if (hi_vdec_check_pic_size(type, width, height) == HI_FALSE) {
        return 0;
    }
