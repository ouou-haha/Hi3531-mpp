...
typedef enum {
    ...
    HI_CIPHER_ALG_SM1,                  /* SM1 algorithm. */
    HI_CIPHER_ALG_SM4,                  /* SM4 algorithm. */
    HI_CIPHER_ALG_DMA,                  /* DMA copy. */
    HI_CIPHER_ALG_BUTT,
...
typedef struct {
    ...
    hi_cipher_ctrl_chg_flag chg_flags;
} hi_cipher_ctrl_aes;
...
typedef struct {
    ...
    hi_u32 iv[HI_CIPHER_MAX_IV_SIZE_IN_WORD];      /* Initialization vector (IV) */
    hi_cipher_key_len key_len;           /* Key length */
    hi_u32 iv_len;                       /* IV length for CCM/GCM, which is an element of {7, 8, 9, 10, 11, 12, 13}
                                            for CCM, and is an element of [1-16] for GCM. */
    hi_u32 tag_len;                      /* Tag length for CCM which is an element of {4, 6, 8, 10, 12, 14, 16}. */
...
typedef struct {
    ...
    hi_u32 byte_len;                /* cipher data length. */
